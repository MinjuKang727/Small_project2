...
class FontInstallerApp:
    def __init__(self, root):
        ...
        sub_label = tk.Label(main_frame, text="폴더 내 폰트 및 압축 파일을 탐색하여 윈도우에 자동 설치합니다. (중복 자동 패스)", font=("Segoe UI", 9), bg="#f8fafc", fg="#64748b")
        ...
        # 📌 압축 파일 자동 해제 여부를 결정하는 체크박스 추가
        self.extract_var = IntVar(value=1) # 기본값: 체크됨 (사용함)
        chk_extract = Checkbutton(
            card_frame, 
            text="📁 폴더 내 압축 파일(ZIP 등) 자동 해제 후 설치 포함", 
        ...
        chk_extract.pack(anchor="w", padx=16, pady=(12, 0))
    ...
    def extract_zip_files(self, source_path):
        """지정한 폴더 내의 zip 파일을 찾아 하위에 자동으로 압축 해제"""
        zip_files = list(source_path.glob("**/*.zip"))
        if not zip_files:
            return

        self.log_box.insert(tk.END, f"📦 총 {len(zip_files)}개의 압축 파일(ZIP)을 발견하여 해제를 시작합니다...\n")
        self.root.update()

        for zip_path in zip_files:
            try:
                # 압축 파일 이름으로 된 폴더를 만들어 그 안에 해제
    ...
    def install_fonts(self):
        """폰트 탐색, 중복 검사, 시스템 설치를 수행하는 핵심 메서드"""
        if not is_admin():
        ...
        # 1. 체크박스가 켜져 있다면 압축 파일 자동 해제 먼저 수행
        if self.extract_var.get() == 1:
            self.extract_zip_files(source_path)

        # 2. 하위 폴더까지 .ttf, .otf 파일 재귀적 탐색 (해제된 폴더 내부 포함)
        font_files = []
        ...
        for font_file in font_files:
            try:
                ...
                # 이미 설치되어 있는지 확인 (파일 존재 여부 + 레지스트리 등록 여부)
        ...
        # 5. 윈도우 시스템 전체에 폰트 변경 브로드캐스트 전송 (재부팅 방지)
